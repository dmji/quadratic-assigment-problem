					...
					case "EUC_2D":
						{
							...
							{
								var aDataStr = aStr[i].Split(' ', StringSplitOptions.RemoveEmptyEntries);
								if(aDataStr.Length < 3)
								{
									i--;
									continue;
								}
								x.Add(double.Parse(aDataStr[1].Trim().Replace('.', ',')));
								y.Add(double.Parse(aDataStr[2].Trim().Replace('.', ',')));
							}
