...
namespace Solution
{
    public partial class TestSystem
    {
        static List<string> getArrtibuteDirFiles(XmlReader xml,string attrName, string path, string ext)
        {
            ...
            RegularSTR regExt = new RegularSTR(ext), reg = new RegularSTR(regStr);
        ...
        {
            tbl.addCells("boldGrey", aOptions[0].getValuesNames().Replace("DEFINE_", "").Split(';', StringSplitOptions.RemoveEmptyEntries));
        ...
        static void initEvolution(string path, ref List<TestInfo> aTest, ref List<IOptions> aOptions, ref Logger log, ref ITabler tbl)
        {
            ...
            log = new Logger(pathLog, $"{xml.GetAttribute("name")}_{EvolutionAlgorithm.getName(true)}");


            string pathTable = path + xml.GetAttribute("pathTable");
            string pathTemplate = path + xml.GetAttribute("pathTemplate");
            tbl = new Tabler(pathTable, $"{xml.GetAttribute("name")}_{EvolutionAlgorithm.getName(true)}", pathTemplate);
        }
